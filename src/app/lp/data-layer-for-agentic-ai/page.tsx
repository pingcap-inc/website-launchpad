...
const schema = buildPageSchema({
  ...
  breadcrumbs: [
    { name: 'Home', path: '/' },
    { name: "The Data Layer for <span class=\"text-gradient-violet\">Agentic AI</span>", path: "/lp/data-layer-for-agentic-ai/" },
  ],
})
  ...
  "sections": [
        ...
        "items": [
          {
            ...
            "description": "You sized the cluster for peak human traffic and a quiet overnight window. Agents run that window at the same rate as noon. Every operation you deferred now has to happen under load, making online schema change an architectural requirement.",
            "image": {
              "image": {
          ...
          },
          {
            ...
            "description": "Agent platforms add a tenant per agent, not per customer. Object count climbs with usage instead of headcount. Tables, schemas, and metadata grow with the tenant list — and the catalog usually breaks before the query path does.",
            "image": {
              "image": {
          ...
          },
          {
            ...
            "description": "If your operational store and retrieval store are kept in step by a pipeline, there is a window where an agent reasons from data that is no longer true. A person notices. An agent acts, then acts again on the result. Closing that window is a consistency problem, not a model problem.",
            "image": {
              "image": {
    ...
    },
    {
      ...
      "props": {
        ...
        "subtitle": "These are the capabilities teams need as agent workloads scale, in the order they hit them.",
        "items": [
          {
            "icon": "Wrench",
            "title": "Schema Changes Without a Window",
            "description": "Because there is no longer a window to use. Online DDL becomes a requirement, not a convenience."
          },
          {
            "icon": "Layers",
            "title": "Tenant Isolation at Massive Scale",
            "description": "Holds at hundreds of thousands of tenants, without a cluster per customer. The catalog must handle it."
          },
          {
            "icon": "RefreshCw",
            "title": "One Consistent View of State",
            "description": "Retrieval and transaction read the same thing at the same moment. No stale reads, no pipeline lag."
          },
          {
            "icon": "TrendingUp",
            "title": "Capacity That Moves Without Migration",
            "description": "Agent load arrives faster than a re-platform can be planned. Scale must be elastic and horizontal."
          }
        ],
        "columns": 2,
        "itemLayout": "vertical",
        ...
        "items": [
          {
            ...
            "title": "Manus",
            "description": "Went from viral launch to more than a million database tenants, and migrated to TiDB Cloud in two weeks to keep up with it. TiDB now holds the context persistence behind Manus's agent swarms and full-stack app generation.",
            "stats": [
              {
                "value": "1M+",
          ...
          },
          {
            ...
            "title": "Kimi",
            "description": "K2.6 builds and hosts full-stack web applications for millions of users, provisioning an isolated database per site in under a second. More than 10 million tenants supported.",
            "stats": [
              {
                "value": "10M+",
                "label": "Tenants"
          ...
          },
          {
            ...
            "title": "Dify",
            "description": "Replaced roughly 500,000 database containers with a single TiDB Cloud instance, unifying vectors, documents, and relational data behind its open-source LLM platform, and cut operational overhead by 90%.",
            "stats": [
              {
                "value": "500K",
          ...
          },
          {
            ...
            "title": "Plaud",
            "description": "Migrated from MySQL plus Amazon S3 to TiDB Cloud, eliminating S3 retrieval latency and unlocking online DDL for 2M+ users across 170 countries. 10x QPS improvement under peak load, 13 clusters in production.",
            "stats": [
              {
                "value": "10x",
    ...
    },
    {
      ...
      "props": {
        ...
        "subtitle": "These are the scenarios where TiDB is not the right fit. If any of these describe your workload, consider alternatives.",
        "items": [
          {
            "icon": "Server",
            "title": "Single-Node Workload",
            "description": "If you run one service with one workload and no multi-tenancy, a single-node database is simpler and you should keep it."
          },
          {
            "icon": "Search",
            "title": "Pure Vector Search",
            "description": "If your only requirement is vector similarity search at the lowest possible latency, a dedicated vector store will be faster."
          },
          {
            "icon": "Code2",
            "title": "MySQL Edge Cases",
            "description": "If your application depends on MySQL behaviour outside the supported surface, that needs checking before a migration, not after."
          },
          {
            "icon": "Gauge",
            "title": "Single-Row High Volume",
            "description": "Distributed transactions cost coordination latency. If your workload is single-row reads at very high volume and consistency does not matter, you are paying for something you do not need."
          }
        ],
        "columns": 2,
        "borderStyle": "gray"
    ...
    },
    {
      ...
      "props": {
        ...
        "subtitle": "Spend thirty minutes with an engineer who has seen this shape of workload before. Bring your current topology and tenant count. You will leave with an answer on whether this is worth your time, including if the answer is no.",
        "image": {
          "image": {
