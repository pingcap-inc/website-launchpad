...
export const metadata: Metadata = {
  ...
  description: "Learn how TiDB silences noisy neighbors in multi-tenant environments, ensuring consistent performance and reliability.",
...
const schema = buildPageSchema({
  ...
  breadcrumbs: [
    { name: 'Home', path: '/' },
    { name: "Silence the Screams: TiDB’s Noisy-Neighbor Playbook", path: "/playbook/noisy-neighbor-multi-tenant-mysql/" },
  ],
})

const dsl: PageDSL = {
  "pageName": "Silence the Screams: TiDB’s Noisy-Neighbor Playbook",
  "meta": {
    "title": "TiDB's Noisy-Neighbor Playbook | TiDB",
    "description": "Learn how TiDB silences noisy neighbors in multi-tenant environments, ensuring consistent performance and reliability.",
  ...
  "sections": [
    {
      ...
      "props": {
        "layout": "image-right",
        "headline": "Silence the Screams: TiDB’s Noisy-Neighbor Playbook",
        "heroImage": {
          "image": {
            "url": "https://static.pingcap.com/images/de560422-1000011706.svg",
            "alt": "1000011706",
            "width": 393,
            "height": 343
          },
    ...
    },
    {
      ...
      "props": {
        "items": [
          ...
          {
            "id": "what-we-re-solving-today-the-noisy-neighbor-spiral",
            "label": "What We’re Solving Today: The Noisy-Neighbor Spiral",
            "level": 1
          },
          {
            "id": "tenant-a-floods-the-cluster-slas-melt",
            "label": "Tenant A Floods the Cluster — SLAs Melt",
            "level": 2
          },
          {
            "id": "symptoms-to-recognize",
            "label": "Symptoms to Recognize",
            "level": 2
          },
          {
            "id": "typical-pressure-paths",
            "label": "Typical Pressure Paths",
            "level": 2
          },
          {
            "id": "hard-isolation-silo-sprawl",
            "label": "Hard Isolation = Silo Sprawl & $$$",
            "level": 2
          },
          {
            "id": "why-costs-explode",
            "label": "Why Costs Explode",
            "level": 2
          },
          {
            "id": "functional-trade-offs",
            "label": "Functional Trade-Offs",
            "level": 2
          },
          {
            "id": "fire-drill-ops-on-call-burnout",
            "label": "Fire-Drill Ops & On-Call Burnout",
            "level": 2
          },
          {
            "id": "what-it-feels-like-in-practice",
            "label": "What It Feels Like in Practice",
            "level": 2
          },
          {
            "id": "diy-isolation-without-tidb-more-metal-more-headaches",
            "label": "DIY Isolation (Without TiDB): More Metal, More Headaches",
            "level": 1
          },
          {
            "id": "carve-siloed-databases-for-every-tenant",
            "label": "Carve Siloed Databases for Every Tenant",
            "level": 2
          },
          {
            "id": "hand-tune-os-vm-and-proxy-limits",
            "label": "Hand-Tune OS, VM, and Proxy Limits",
            "level": 2
          },
          {
            "id": "rewrite-app-logic-for-cross-tenant-joins",
            "label": "Rewrite App Logic for Cross-Tenant Joins",
            "level": 2
          },
          {
            "id": "over-provision-or-pray-during-spikes",
            "label": "Over-Provision or Pray During Spikes",
            "level": 2
          },
          {
            "id": "tidb-s-multi-tenant-shield-with-tidb",
            "label": "TiDB’s Multi-Tenant Shield (With TiDB)",
            "level": 1
          },
          {
            "id": "resource-groups-cpu-io-budgets-per-tenant-in-seconds",
            "label": "Resource Groups: CPU & IO Budgets per Tenant in Seconds",
            "level": 2
          },
          {
            "id": "placement-rules-pin-hot-data-keep-cold-cheap",
            "label": "Placement Rules: Pin Hot Data, Keep Cold Cheap",
            "level": 2
          },
          {
            "id": "fine-grained-role-based-access-control-rbac-least-privilege-zero-app-changes",
            "label": "Fine-Grained Role-Based Access Control (RBAC): Least Privilege, Zero App Changes",
            "level": 2
          },
          {
            "id": "shared-hardware-isolated-performance",
            "label": "Shared Hardware, Isolated Performance",
            "level": 2
          },
          {
          ...
          {
            "id": "assess-hotspots-tenant-qos",
            "label": "Assess Hotspots & Tenant QoS",
            "level": 2
          },
          {
            "id": "configure-resource-groups-placement-labels",
            "label": "Configure Resource Groups & Placement Labels",
            "level": 2
          },
          {
            "id": "rollout-monitor-optimize",
            "label": "Rollout, Monitor, Optimize",
            "level": 2
          },
          {
          ...
          {
            "id": "tidb-dashboard-heatmaps-top-sql-tenant-lens",
            "label": "TiDB Dashboard: Heatmaps, Top SQL, Tenant Lens",
            "level": 2
          },
          {
            "id": "backups-with-resource-throttling",
            "label": "Backups with Resource Throttling",
            "level": 2
          },
          {
            "id": "capacity-forecasts-cost-guardrails",
            "label": "Capacity Forecasts & Cost Guardrails",
            "level": 2
          },
          {
            "id": "proving-noisy-neighbor-isolation-tenants-a-b",
            "label": "Proving Noisy-Neighbor Isolation (Tenants A & B)",
            "level": 2
          },
          {
            "id": "1-minimal-setup",
            "label": "1. Minimal Setup",
            "level": 2
          },
          {
            "id": "2-prepare-data-with-sysbench-run-once-per-tenant",
            "label": "2. Prepare Data with sysbench (Run Once Per Tenant)",
            "level": 2
          },
          {
            "id": "tenant-a-baseline-no-isolation-expect-interference",
            "label": "Tenant A Baseline: No Isolation (Expect Interference)",
            "level": 2
          },
          {
            "id": "tenant-b-baseline-enable-isolation-with-resource-groups-seconds",
            "label": "Tenant B Baseline: Enable isolation with Resource Groups (seconds)",
            "level": 2
          },
          {
            "id": "what-to-watch-during-the-run",
            "label": "What to Watch During the Run",
            "level": 2
          },
          {
            "id": "backups-and-maintenance-that-respect-budgets",
            "label": "Backups and Maintenance that Respect Budgets",
            "level": 2
          },
          {
            "id": "capacity-forecasts-guardrails",
            "label": "Capacity Forecasts & Guardrails",
            "level": 2
          },
          {
            "id": "why-tidb-handles-noisy-neighbors-well",
            "label": "Why TiDB Handles Noisy Neighbors Well",
            "level": 1
          },
          {
          ...
          {
            "id": "atlassian-from-hundreds-of-postgres-clusters-to-16-tidb-clusters-with-50-efficiency",
            "label": "Atlassian: From Hundreds of Postgres Clusters to 16 TiDB Clusters with 50× Efficiency",
            "level": 2
          },
          {
            "id": "outcomes",
          ...
          {
            "id": "plaid-faster-upgrades-higher-uptime-non-disruptive-scale",
            "label": "Plaid: Faster Upgrades, Higher Uptime, Non-Disruptive Scale",
            "level": 2
          },
          {
            "id": "next-steps",
          ...
          {
            "id": "launch-a-free-tidb-cluster",
            "label": "Launch a Free TiDB Cluster",
            "level": 2
          },
          {
            "id": "book-a-multi-tenant-architecture-workshop",
            "label": "Book a Multi-Tenant Architecture Workshop",
            "level": 2
          }
        ],
        "sticky": true
