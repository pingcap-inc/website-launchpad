...
function buildListiclePrompt(dsl: PageDSL): { system: string; user: string } {
  ...
  const user = [
    ...
    '- Each numbered item has a distinct, specific description (not generic)',
    '- Items cover key differentiators: use cases, pros/cons, pricing signals',
    '- Body content is substantive (not just 1-2 sentences per item)',
    ...
    '- Items have sectionTitle/badge that help AI understand the page taxonomy',
    '',
    '[Structure - 30 points]',
    '- Page follows 4-section skeleton: richTextBlock → numberedList → faq → cta',
    '- meta title (50–60 chars) and description (120–160 chars) are present',
    '- pageName is present (used as H1)',
    '- numberedList items have id attributes (required for ItemList schema anchors)',
    '- No extra sections beyond the standard 4 (richTextBlock, numberedList, faq, cta)',
    '',
    'Guidance:',
    '- DO NOT flag heading hierarchy (H1/H2/H3) as an issue. The component system automatically maps hero.headline → H1, section titles → H2, and subheadlines → H3. This is already correct.',
